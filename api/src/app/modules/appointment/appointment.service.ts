import { Appointments, Customer, Payment, paymentStatus } from "@prisma/client";
...
const createAppointment = async (payload: any): Promise<Appointments | null | any> => {
    ...
    if(patientInfo.customerId){
        const isUserExist = await prisma.customer.findUnique({
            where: {
                id: patientInfo.customerId
            }
        })
        if (!isUserExist) {
            patientInfo['customerId'] = null
        }
    }

    const isDoctorExist = await prisma.gardener.findUnique({
        where: {
            id: patientInfo.gardenerId
        }
    });
    ...
    const result = await prisma.$transaction(async (tx) => {
        ...
        const appointmentObj = {
            ...
            doctorImg: appointment?.gardener?.img,
            doctorFirstName: appointment?.gardener?.firstName,
            doctorLastName: appointment?.gardener?.lastName,
            specialization:appointment?.gardener?.specialization,
            designation:appointment?.gardener?.designation,
            college:appointment?.gardener?.college,
            patientImg:appointment?.customer?.img,
            patientfirstName:appointment?.customer?.firstName,
            patientLastName:appointment?.customer?.lastName,
            dateOfBirth: moment().diff(moment(appointment?.customer?.dateOfBirth), 'years'),
            customerGroup:appointment?.customer?.customerGroup,
        ...
        const subject = `Appointment Confirm With Dr ${appointment?.gardener?.firstName + ' ' + appointment?.gardener?.lastName} at ${appointment.scheduleDate} + ' ' + ${appointment.scheduleTime}`
        const toMail = `${appointment.email + ',' + appointment.gardener?.email}`;
...
const createAppointmentByUnAuthenticateUser = async (payload: any): Promise<Appointments | null> => {
    ...
    const result = await prisma.$transaction(async (tx) => {
        ...
        patientInfo['trackingId'] = trackingId;
        patientInfo['gardenerId'] = config.defaultAdminDoctor
...
const getPatientAppointmentById = async (user: any): Promise<Appointments[] | null> => {
    const { userId } = user;
    const isPatient = await prisma.customer.findUnique({
        where: {
            id: userId
        }
    })
...
const getPatientPaymentInfo = async (user: any): Promise<Payment[]> => {
    ...
    const result = await prisma.payment.findMany({
        where: { appointment: { customerId: isUserExist.id } },
        include: {
            appointment: {
                include: {
                    gardener: {
...
const getDoctorInvoices = async (user: any): Promise<Payment[] | null> => {
    ...
    const result = await prisma.payment.findMany({
        where: { appointment: { gardenerId: isUserExist.id } },
        include: {
            appointment: {
                include: {
                    customer: {
...
const getDoctorAppointmentsById = async (user: any, filter: any): Promise<Appointments[] | null> => {
    ...
    if (!isDoctor) { throw new ApiError(httpStatus.NOT_FOUND, 'Gardener Account is not found !!') }

    let andCondition: any = { gardenerId: userId };
...
const getDoctorPatients = async (user: any): Promise<Customer[]> => {
    const { userId } = user;
    const isDoctor = await prisma.gardener.findUnique({
        where: {
            id: userId
    ...
    const patientIds = patients.map(appointment => appointment.customerId);
    const patientList = await prisma.customer.findMany({
        where: {
            id: {
                // @ts-ignore
                in: patientIds
