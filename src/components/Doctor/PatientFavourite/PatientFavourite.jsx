            ...
            {data && data?.map((item) => (
                <div className="col-md-4 col-sm-12 mb-4 mx-2 rounded shadow-sm" key={item?.id} style={{ background: '#f8f9fa', maxWidth: '20rem' }}>
                    <div className='rounded position-relative'>
                        ...
                        <div style={{ cursor: 'pointer' }} className="m-2 text-success position-absolute top-0 end-0 me-2" onClick={() => handleRemoveFavourite(item?.gardener?.id)}>
                            <FaBookmark />
                        </div>

                        <div className='p-2'>
                            <h5 className="d-flex align-items-center gap-2 mb-0" style={{ color: '#1977cc' }}>
                                <Link to={`/doctors/profile/${item?.gardener?.id}`}>
                                    {item?.gardener?.firstName + ' ' + item?.gardener?.lastName}
                                </Link>
                                <FaCheckCircle className='verified text-success' />
                            </h5>
                            <p className="form-text">MBBS, MD - General Cleaning</p>
                            ...
                            <div className="d-flex justify-content-between mb-3 mt-2">
                                ...
                                <Link to={`/booking/${item?.gardener?.id}`} className="btn book-btn" style={{ backgroundColor: '#1977cc' }}>Book Now</Link>
                            </div>
                        </div>
                    </div>
                </div>
