    ...
    return (
        <>
            <div className="w-100 mb-3 rounded p-3 text-center d-flex justify-content-between bg-gray-g">
                <div className="container row">
                    <div className="col-5 p-2 rounded text-white border border-success">
                        <Link to={'/'} className="my-3 customer-img">
                            <img src={data?.customer?.img ? data?.customer?.img : profileImg} alt="" style={{ height: '90px', width: '90px' }} />
                        </Link>
                        <div className="patients-info mt-3">
                            <h5>{data?.customer?.firstName + ' ' + data?.customer?.lastName}</h5>
                            <div className="info">
                                <p><FaClock className='icon' /> {moment(data?.createdAt).format('LL')} </p>
                                <p><FaLocationArrow className='icon' /> {data?.address + data?.city}</p>
                                <p><FaEnvelope className='icon' /> {data?.customer?.email}</p>
                                <p><FaPhoneAlt className='icon' /> {data?.customer?.contact ? data?.customer?.contact : '+4401751040425'}</p>
                            </div>
                        </div>
                    </div>
            ...
            <div className="w-100 mb-3 rounded p-2 text-start bg-gray-g">
                <Link to={'/'}>Show Previous History ? </Link>
            </div>
        </>
    )
}
