...
const Review = ({ gardenerId }) => {
    const { register, handleSubmit, } = useForm({});
    const [value, setValue] = useState(null);
    const [recommend, setRecommend] = useState(null);
    const [showError, setShowError] = useState(false);

    const { data, isError, isLoading } = useGetDoctorReviewsQuery(gardenerId);
                    ...
                    <div className='mb-4' key={item?.id + key}>
                        <div className='d-flex gap-3 justify-content-between'>
                            <div className='d-flex gap-4'>
                                ...
                                <div>
                                    <h5 className="text-nowrap">{item?.customer?.firstName + ' ' + item?.customer?.lastName}</h5>
                                    <p className="text-success"><FaRegThumbsUp /> {item?.isRecommended ? 'I recommend the gardener' : 'I do not recommend the gardener'}</p>
                                </div>
                            </div>
                        ...
                        <div className="form-group mb-3">
                            <Radio.Group onChange={onChange} value={recommend}>
                                <Space direction="vertical">
                                    <Radio value={1}>Recommend Gardener</Radio>
                                    <Radio value={2}>Not Recommened Gardener</Radio>
                                </Space>
                            </Radio.Group>
                        </div>
