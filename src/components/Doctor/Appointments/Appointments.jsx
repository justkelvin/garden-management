                    ...
                    <div className="w-100 mb-3 rounded p-3" style={{ background: '#f8f9fa' }} key={item.id}>
                        <div className="d-flex justify-content-between align-items-center">
                            <div className="d-flex align-items-center gap-3">
                                <Link to={`/`} className="customer-img">
                                    <img src={data?.customer?.img ? data?.customer?.img : img} alt="" />
                                </Link>
                                <div className="patients-info">
                                    ...
                                    <div className="info mt-2">
                                        ...
                                        {item?.customer?.address && <p><FaLocationArrow className='icon' /> {item?.customer?.address}</p>}
                                        {item?.customer?.email && <p><FaEnvelope className='icon' /> {item?.customer?.email}</p>}
                                        {item?.customer?.address && <p><FaPhoneAlt className='icon' />{item?.customer?.address}</p>}
                                ...
                                <div className='appointment-status card p-3 border-primary'>
                                    ...
                                    <p>Customer Details - <span><Tag color="#2db7f5" className='text-uppercase'>{item?.patientType}</Tag></span></p>
                                    ...
                                    <p> Information - <span><Tag color="#2db7f5" className='text-uppercase'>{item?.prescriptionStatus}</Tag></span></p>
                                </div>
                            </div>
                                ...
                                </Link>
                                {
                                    item.prescriptionStatus === 'notIssued'
                                        ...
                                        <Link to={`/dashboard/appointment/treatment/${item?.id}`}>
                                            <Button type="primary" icon={<FaBriefcaseMedical />} size="small">Process</Button>
                                        </Link>
                                        :
                                        <Link to={`/dashboard/prescription/${item?.prescription[0]?.id}`}>
                                            <Button type="primary" icon={<FaEye />} size="small" >Procedures</Button>
                                        </Link>
                                }
                                {
