    ...
    const columns = [
        {
            ...
            render: function (data) {
                const fullName = `${data?.appointment?.customer?.firstName ?? ''} ${data?.appointment?.customer?.lastName ?? ''}`;
                const patientName = fullName.trim() || "Private Customer";
                return (
                    <div className="table-avatar">
                        <a className="avatar avatar-sm mr-2 d-flex gap-2">
                            <img className="avatar-img rounded-circle" src={data?.appointment?.customer?.img ? data?.appointment?.customer?.img : img} alt="" />
