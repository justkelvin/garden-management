...
const DoctorBooking = () => {
    ...
    const [customerId, setPatientId] = useState('');
    ...
    const { data, isLoading, isError, error } = useGetDoctorQuery(gardenerId);
    const { data: time, refetch, isLoading: dIsLoading, isError: dIsError, error: dError } = useGetAppointmentTimeQuery({ day: selectDay, id: gardenerId });
    ...
    if (!dIsLoading && !dIsError && time.length === 0) dContent = <Empty children="Service Is not Available" />
    ...
    const steps = [
        {
            title: 'Select Service Date & Time',
    ...
    const handleConfirmSchedule = () => {
        ...
        obj.patientInfo = {
            ...
            customerId: role !== '' && role === 'customer' ? customerId : undefined,
        }
        obj.payment = {
    ...
    useEffect(() => {
        if (createIsSuccess) {
            message.success("Succcessfully Scheduled")
